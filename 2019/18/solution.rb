...
def h_h keys
  (@keys.length + 1 - keys.length) * 5
end

def is_goal_goal? current_keys
  current_keys.length == @keys.count
end
...
def get_neighbors_neighbors current
  ...
  potentials = points.each_with_index.map do |point, i|
    @keys_by_quad[i].select do |key|
      position = @keys[key]
      if point == key
        false
        ...
        !(remaining_keys.any? do |k|
          ...
          door_pos = @doors[door]
          key_pos = @keys[k]
          # puts "distance for these two: #{point} and #{key}"
  ...
  # puts potentials.to_s

  potentials
end

def score_for_neighbor_neighbor current, index, neighbor
  @distances[[current.last[index], neighbor]][1]
end
...
def a_star_star start
  ...
  while set.length > 0
    ...
    current_keys, current_locations = current

    return [reconstruct_path(from,current), g[current]] if is_goal_goal?(current_keys)
    ...
    neighbors.each_with_index do |new_points, i|
      # puts "Checking #{new_point} as a neighbor for #{curr_path}"
      new_points.each do |new_point|
        tent_g = g[current]

        tent_g += score_for_neighbor_neighbor(current, i, new_point)

        points = current_locations.clone
        points[i] = new_point
        new_key_key = [current_keys + Set.new([new_point]), points]
  ...
  0.upto(grid.length - 1) do |y|
    ...
    if /[[:upper:]]/.match(input)
      ...
      if x < grid[0].length / 2 && y < grid.length / 2
        @keys_by_quad[0] ||= []
        @keys_by_quad[0] << input
      elsif x > grid[0].length / 2 && y < grid.length / 2
        @keys_by_quad[1] ||= []
        @keys_by_quad[1] << input
      elsif x > grid[0].length / 2 && y > grid.length / 2
        @keys_by_quad[2] ||= []
        @keys_by_quad[2] << input
      elsif x < grid[0].length / 2 && y > grid.length / 2
        @keys_by_quad[3] ||= []
        @keys_by_quad[3] << input
      end
...
0.upto(3) do |quad|
  @keys_by_quad[quad].each do |key|
    @keys_by_quad[quad].each do |goal|
      next if key == goal
      next if @distances[[key, goal]] || @distances[[goal, key]]
      @goal = [goal, @keys[goal]]
      result = a_star(@keys[key])
